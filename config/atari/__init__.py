...
from core.utils import make_atari, WarpFrame, EpisodicLifeEnv, DelayWrapper
...
class AtariConfig(BaseConfig):
    ...
    def set_game(self, env_name, save_video=False, save_path=None, video_callable=None):
        ...
        self.env_name_short = (env_name.split('NoFrameskip'))[0]
        # gray scale
        if self.gray_scale:
    ...
    def new_game(self, seed=None, save_video=False, save_path=None, video_callable=None, uid=None, test=False, final_test=False, delay_enable=False):
        if test:
            if final_test:
        ...
        if delay_enable:
            env = DelayWrapper(env, max_delay_value=self.delay)
