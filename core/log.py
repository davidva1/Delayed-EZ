...
def _log(config, step_count, log_data, model, replay_buffer, lr, shared_storage, summary_writer, vis_result):
    ...
    worker_ori_reward, worker_reward, worker_reward_max, worker_eps_len, worker_eps_len_max, test_counter, test_dict,\
    temperature, visit_entropy, priority_self_play, distributions, processed_eps_lengths, predict_errors,\
    delay_values = worker_logs
    ...
    wandb.log({"Total Loss": total_loss}, step=step_count)

    # Forward Prediction errors
    #for i in range(len(predict_errors)):
    #    wandb.log({"Predict Err": predict_errors[i]})

    # Visualize stochastic delays values
    for i in range(len(delay_values)):
        wandb.log({"Delay Values ": delay_values[i]})

    # Episode lengths histograms
    if len(processed_eps_lengths) > 0:
        wandb.log({"Eps Length Processed":wandb.Histogram(processed_eps_lengths)})
    ...
    if test_dict is not None:
        ...
        test_msg = '#{:<10} Test Mean Score of {}: {:<10} (max: {:<10}, min:{:<10}, std: {:<10})' \
                   ''.format(test_counter, config.env_name_short, mean_score, max_score, min_score, std_score)
        test_logger.info(test_msg)
        print('test dict is not None', mean_score)

        wandb.log({"(Delay) Mean score": mean_score,
                   "(Delay) Median score": median_score})
