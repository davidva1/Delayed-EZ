...
class BatchWorker_CPU(object):
    ...
    def make_batch(self, batch_context, ratio, weights=None):
        ...
        while i < batch_size:
            ...
            obs = game_lst[i].obs(game_pos_lst[i], extra_len=self.config.num_unroll_steps, padding=True)
            #if obs is None:
                # don't prepare a batch if the batch context has empty obs
            #    print('Skipping batch context since obs is None')
            #    return
            obs_lst.append(obs)
