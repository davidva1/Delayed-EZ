...
def _train(model, target_model, replay_buffer, shared_storage, batch_storage, config, summary_writer):
    ...
    print(ray.get(replay_buffer.get_total_len.remote()), config.start_transitions)
    ...
    print(config.training_steps)
    print(config.training_steps + config.last_steps)
    # while loop
    while step_count < config.training_steps + config.last_steps:
        ...
        if (step_count+1)%2000 == 0:
            print('step count=', step_count+1)
        ...
        if step_count % config.save_ckpt_interval == 0:
            model_path = os.path.join(config.model_dir, 'model_{}_{}_seed={}_{}_{}.p'.format(step_count, config.env_name_short,
                                                                                          config.seed, config.start_time,
                                                                                          socket.gethostname()))
            torch.save(model.state_dict(), model_path)
            if step_count >= 140000:
                print(f'Saving model with wandb, step = {step_count}, path = {model_path}')
