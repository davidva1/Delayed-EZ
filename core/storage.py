...
class SharedStorage(object):
    ...
    def get_worker_logs(self):
        if len(self.reward_log) > 0:
            ...
            # For delay
            processed_eps_lengths = self.processed_eps_lengths
            predict_errors = self.predict_errors
            delay_values = self.delay_values
        ...
        return ori_reward, reward, reward_max, eps_lengths, eps_lengths_max, test_counter, test_dict, temperature,\
               visit_entropy, priority_self_play, distributions, processed_eps_lengths, predict_errors, delay_values

    def set_delay_logs(self, processed_eps_length=None, predict_errors=None, current_delay_value=None):
        if processed_eps_length is not None:
            self.processed_eps_lengths.append(processed_eps_length)
        if predict_errors is not None:
            self.predict_errors.append(predict_errors)
        if current_delay_value is not None:
            self.delay_values.append(current_delay_value)
